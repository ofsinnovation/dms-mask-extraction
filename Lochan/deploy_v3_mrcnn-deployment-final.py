...
class InferenceConfig(Config):
    ...
    DEFAULT_WEIGHT_FILE_PATH = '/home/ofsdms/pan_document_extraction_30.h5'
     
    # For aadhaar_object_extraction
    # DEFAULT_API_NAME = '/aadhaar_extraction'

    # For pan_object_extraction
    DEFAULT_API_NAME = '/pan_document_extraction'
...
def get_item_of_interest(detections, items_required, image):
    ...
    if items_required is None:
        print('Items_required is NONE')
        items_required = config_file.ITEM_OF_INTEREST
        print('type of items_required '+str(type(items_required)))
        if items_required not in items:
            print('Item is not in items_required :'+str(item))
...
def get_detections():
    ...
    print(image)
    print("Entering get_detection api call")    
    ...
    req_result = get_item_of_interest(detections, items_of_interest, image)
    print(req_result)
    return req_result
